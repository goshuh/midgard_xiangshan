...
class TLB(Width: Int, nRespDups: Int = 1, q: TLBParameters)(implicit p: Parameters) extends TlbModule with HasCSRConst with HasPerfEvents {
  ...
  def TLBNormalRead(i: Int) = {
    ...
    val vaddr = Mux(vmEnable_dup(i), SignExt(req(i).bits.vaddr, PAddrBits), ZeroExt(req(i).bits.vaddr, PAddrBits))
    ...
    resp(i).bits.priv := mode_dup(i) =/= ModeU
  ...
  val refill = ptw_resp_v && !sfence_dup.head.valid && !csr_dup.head.satp_changed
  normalPage.w_apply(
