...
class MainPipe(implicit p: Parameters) extends DCacheModule with HasPerfEvents {
  val io = IO(new Bundle() {
    ...
    val store_uat_resp = ValidIO(new DCacheLineResp)
...
//  val amo_bitmask = FillInterleaved(8, s3_req.amo_mask)
  ...
  io.store_uat_resp.valid   := s2_valid && s2_req.isStore
  io.store_uat_resp.bits    := DontCare
  io.store_uat_resp.bits.id := s2_req.id
  ...
  io.error_flag_write.valid := false.B // s3_fire_dup_for_err_w_valid && update_meta_dup_for_err_w_valid && s3_l2_error
  ...
  io.tag_write.bits.tag := Mux(s3_l2_error, 0.U, get_tag(s3_req_addr_dup(4)))
  ...
  io.data_write.valid := s3_valid_dup_for_data_w_valid && s3_update_data_cango_dup_for_data_w_valid && update_data_dup_for_data_w_valid && !s3_l2_error
