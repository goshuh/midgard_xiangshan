...
class RefillPipe(implicit p: Parameters) extends DCacheModule {
  ...
  io.data_write.valid := io.req_dup_for_data_w(0).valid && !err
  ...
  io.data_write_dup.zipWithIndex.foreach { case (w, bank) =>
    w.valid := io.req_dup_for_data_w(bank).valid && !err
  ...
  io.error_flag_write.valid := io.req_dup_for_err_w.valid && !err
